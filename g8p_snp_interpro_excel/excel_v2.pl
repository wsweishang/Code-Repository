...
open (INBLAST,"<G:/count.txt") or die "$!";

my %blast=();
foreach my $blast(<INBLAST>){
	chomp $blast;
	my @blast=split(/\t/,$blast);
	$blast{$blast[0]}="$blast[2]\t$blast[3]\t$blast[1]";
}
...
print OUT "GeneID\t",join ("\t\t",@order),"\t\tInterpro","\tblastp\t","\tD","\n";
