...
####################################################################################################
open (GENELIST, "<D:/Takifugu/2_Data_analysis/Cufflink_without_Dedup/cuffnorm/2_Mfuzz/1_Mfuzz_perl/Takifugu_rubripes_gene_id_comparison_table_v2.txt") or die "$!";
open (FPKMLIST, "<D:/Takifugu/2_Data_analysis/Cufflink_without_Dedup/cuffnorm/2_Mfuzz/1_Mfuzz_perl/Fpkm_pathlist.txt") or die "$!";
my $output_dir = "D:/Takifugu/2_Data_analysis/Cufflink_without_Dedup/cuffnorm/2_Mfuzz";
####################################################################################################
my (%genelist) = ();
while (<GENELIST>) {
	chomp (my $genelist = $_);
