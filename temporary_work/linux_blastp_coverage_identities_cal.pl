...
#open (aa_length_list, "<$ARGV[0]") or die "Input file 0 cannot be opened.\n"; #>seq_id length
#open (blastp, "$ARGV[1]") or die "Input file 0 cannot be opened.\n"; # alignment of gene models to known gene, -m 8
#my $identity_cutoff = $ARGV[2];   # 0 <= $identity_cutoff <= 1
#my $match_len_cutoff = $ARGV[3]; 
#open (OUTPUT2, ">$ARGV[1]_coverage_index") or die "Output file cannot be opened.\n"; 				
#
...
#	my $gene = $tmp[0];
#	my $homolog = $tmp[1];
#	my $gene_homolog = $tmp[0]."XXX".$tmp[1];
#	my $nt = $tmp[3];
#	my $identity = $tmp[2] / 100;
...
#}
#
#my @gene_homolog_list = sort{$a cmp $b} keys %alignedregion;
