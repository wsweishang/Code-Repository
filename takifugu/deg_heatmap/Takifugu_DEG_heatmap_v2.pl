...
####################################################################################################
open (INGENELIST, "<D:/research work/Takifugu/RNAseq/03.statistics/07.heatmap/new/t.txt") or die "$!";
my @deg_pathes = glob ("'D:/research work/Takifugu/RNAseq/03.statistics/01.Cufflink/cuffdiff/*/gene_exp.diff'");
my $output_dir = "D:/research work/Takifugu/RNAseq/03.statistics/07.heatmap/new/";
####################################################################################################
...
while (<INGENELIST>) {
	...
	next if ($data =~ /^SUID.*/);
	my @data = split (/\t/, $data);
	my @gene_name = split (/_/, $data[0]);
	$gene_list{$data[0]} = $gene_name[0];
	push (@gene_list, $data[0]);
}
close (INGENELIST);
...
foreach my $deg_path (@deg_pathes) {
	...
	while (<INDEG>) {
		...
		my @data = split (/\t/, $data);
		foreach my $gene_list (@gene_list) {
			if ($data[2] eq $gene_list{$gene_list}) {
				...
				$deg_gene{$organ}{"$data[0]\t$gene_list"}{$population}{$temprature} = "$fold_change\t$data[12]";
			}
		}
	}
	close (INDEG);
...
foreach my $organ (sort {$a cmp $b} keys %deg_gene) {
	...
	print OUT "gene_id\tgene_name\tTA_04\tTA_04_fdr\tTA_07\tTA_07_fdr\tTA_16\tTA_16_fdr\tTU_04\tTU_04_fdr\tTU_07\tTU_07_fdr\tTU_16\tTU_16_fdr\n";
