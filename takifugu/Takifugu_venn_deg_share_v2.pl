...
open (PATHLIST, "<D:/research work/Takifugu/RNAseq/03.statistics/02.WGCNA/Fugu_3/TATU_updown/Takifugu_rubripes_TA_vs_TU_DEG_pathlist.txt") or die "$!";
#my $output_dir = "D:/research work/Takifugu/RNAseq/03.statistics/02.WGCNA/Fugu_3";
open (OUT, ">D:/research work/Takifugu/RNAseq/03.statistics/02.WGCNA/Fugu_3/TATU_updown/Takifugu_rubripes_TA_vs_TU_DEG_gene_id.txt") or die "$!";
#####################################################################################################
my (%genelist) = ();
while (<GENELIST>) {
	chomp (my $genelist = $_);
...
#				print OUTDOWN "$deg[0]\n";
#			}
			print OUT "$deg[0]\t$genelist{$deg[0]}\n";
		}		
	}
	close (INDEG);
#	close (OUTUP);
