...
my $nbp_between_snp_and_snp = $ARGV[4];
my $nbp_between_snp_and_indel = $ARGV[5];
my $nbp_on_head_and_tail = $ARGV[6];
my $min_snp_valid_depth = $ARGV[7];
my $min_ref_alt_frequency_ratio = $ARGV[8];
print "script filter parameter : 
	snp vcf path : $ARGV[0]
	indel vcf path : $ARGV[1]
	scaffold/chromesome list path : $ARGV[2]
	output snp vcf path : $ARGV[3]
...
print OUT '##FILTER=<ID=NEARIndel,Description="This snp site is near a indel or the head/tail of scaffolds">' . "\n";
