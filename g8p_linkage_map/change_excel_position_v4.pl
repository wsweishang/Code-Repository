...
open (OUT,">G:/LG_1-24_Dist_1-24_with_zero_v3.txt") or die "$!";
...
foreach my $inexcel(<INEXCEL>){
	...
	if ($order{$inexcel[2]}{"orientation"} eq "+"){
		$excel{$order{$inexcel[2]}{"LG"}}{$inexcel[1]}{"scaffold"}=($order{$inexcel[2]}{"startsite"}+$inexcel[3]-1)/500000;
		$excel{$order{$inexcel[2]}{"LG"}}{$inexcel[1]}{"lgcm"}=$inexcel[4];
	}elsif ($order{$inexcel[2]}{"orientation"} eq "-"){
		$excel{$order{$inexcel[2]}{"LG"}}{$inexcel[1]}{"scaffold"}=(($order{$inexcel[2]}{"length"}-$inexcel[3])+$order{$inexcel[2]}{"startsite"})/500000;
		$excel{$order{$inexcel[2]}{"LG"}}{$inexcel[1]}{"lgcm"}=$inexcel[4];
	}
}
close (INEXCEL);
